...
object RemoteGraph {
  ...
  class RemoteGraphMaterializer(useInProcessCluster: Boolean, system: ActorSystem)
    ...
    override def materialize(subGraph: SubGraph, inputMatValues: Map[Module, Any]):
    Map[Module, Any] = {
    ...
    private def doMaterialize(graph: Graph[Module, Edge], inputMatValues: Map[Module, Any]):
    Map[Module, Any] = {
